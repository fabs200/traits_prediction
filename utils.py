...
def extract_model_specification(method='logistic', selected_feature_set=None, depvar=None):
    if method is None:
        ...
        model_ = f"model_{depvar}_{model['method']}_{selected_feature_set[8:]}_ts{model['test_size']}_mss{model['min_samples_split']}_md{model['max_depth']}_msl{model['min_samples_leaf']}_mf{model['max_features']}_ne{model['n_estimators']}_mid{model['min_impurity_decrease']}_GS{model['do_grid_search']}"
    return model_
